...
try:
    if __package__:
        ...
        from .utils.fileio import ensure_dir, read_csv_safe, write_csv_atomic, append_with_rolloff  # type: ignore
        from .utils.numerics import clip_inf_nan  # type: ignore
        from .trading_calendar import get_trading_days  # type: ignore
        from .api.myquant_io import get_ohlcv  # type: ignore
        from . import universe  # type: ignore
        from config import (
            CFG,
            LOG_VERBOSITY,
            FQ,
...
def _retry_call(fn, *args, **kwargs):
    ...
    raise last  # noqa: TRY200

def collect_and_store_ohlcv(codes: List[str], start: str, end: str, fq: str, out_dir: str) -> None:
    ensure_dir(out_dir)
    df = _retry_call(get_ohlcv, codes, start, end, fq)
    if df.empty:
        log.warn(f"[PRE] No OHLCV fetched for window {start}→{end}")
        return
    # split & write per code (append/incremental)
    for code, g in df.groupby("code"):
        g = g.sort_values("date")
        p = os.path.join(out_dir, f"{code}.csv")
        write_csv_atomic(p, g, index=False)

def build_panel_from_csv(codes: List[str], start: str, end: str, raw_dir: str) -> pd.DataFrame:
    """
    从 raw_dir 读取按“规范代码”命名的单股 CSV，拼接为面板，并把 date 向下贴合到交易日历。
    区间最终裁剪到 [start, end]（含），日期标准化到 00:00:00。
    """
    import numpy as np  # 只在本函数内使用，避免外部依赖

    frames: List[pd.DataFrame] = []
    for c in codes:
        p = os.path.join(raw_dir, f"{c}.csv")
    ...
    if not frames:
        # 返回带关键列的空表，避免后续 KeyError
        return pd.DataFrame(columns=[
            "date", "code", "open", "high", "low", "close",
            "volume", "amount", "preclose", "paused", "high_limit", "low_limit"
        ])

    # 拼接与基础清洗
    df_all = pd.concat(frames, ignore_index=True)
    df_all["date"] = pd.to_datetime(df_all["date"], errors="coerce").dt.tz_localize(None).dt.normalize()
    df_all["code"] = df_all["code"].astype(str)

    # —— 用交易日历对齐日期（向下贴合 floor）——
    from .trading_calendar import get_trading_days
    start_dt = pd.to_datetime(start)
    end_dt = pd.to_datetime(end)

    # 日历适当加一点缓冲，避免边界误差
    pad_start = (start_dt - pd.Timedelta(days=5)).strftime("%Y-%m-%d")
    pad_end = (end_dt + pd.Timedelta(days=5)).strftime("%Y-%m-%d")
    cal = pd.DatetimeIndex(
        pd.to_datetime(get_trading_days(pad_start, pad_end), errors="coerce")
    ).tz_localize(None).normalize()

    # searchsorted 向右插入再减 1 等价于 floor 到最近（或相等）的交易日
    dts = df_all["date"].values
    pos = np.searchsorted(cal.values, dts, side="right") - 1
    pos = np.clip(pos, 0, len(cal) - 1)  # ndarray 的正确裁界写法
    df_all["date"] = cal.values[pos]
    ...
    # 数值健壮化（若你的 clip_inf_nan 只对数值列操作，这里可保留；否则可去掉）
    try:
        df_all = clip_inf_nan(df_all)
    except Exception:
        pass
...
def run_preprocess(start: pd.Timestamp, end: pd.Timestamp, warmup_days: Optional[int] = None) -> CoverageRow:
    log.set_verbosity(LOG_VERBOSITY)

    start = pd.Timestamp(start).normalize()
    end = pd.Timestamp(end).normalize()
    WARMUP_DAYS = int(warmup_days or getattr(CFG, "WARMUP_DAYS", 252))

    s_full = start - pd.Timedelta(days=WARMUP_DAYS)
    days_full: List[pd.Timestamp] = get_trading_days(s_full, end)
    days_main: List[pd.Timestamp] = get_trading_days(start, end)
    if not days_full:
        raise RuntimeError("No trading days in the requested window (including warmup).")
    ...
    # ---- build union universe across days_full ----
    # Single-pass union scan across the full warmup+main window
    t0 = time.time()
    codes = universe.rolling_union_codes(start=days_full[0], end=days_full[-1])
    codes = sorted(set(codes))
    t1 = time.time()
    log.done(f"[UNIV] prefetch codes size={len(codes)} time={t1 - t0:.2f}s")
    ...
    log.step(f"[IO] OHLCV fetch symbols={len(codes)} window=[{s_str} -> {e_str}] fq='{FQ}'")
    log.step(f"Fetch OHLCV in batches (BATCH_SIZE={BATCH_SIZE}, fq='{FQ}')")
    N = len(codes)
    t1 = time.time()
    for i in range(0, N, BATCH_SIZE):
        ...
        try:
            ...
            continue
        frames.append(df_b)
        log.loop_progress("OHLCV fetch", min(i + BATCH_SIZE, N), N, start_time=t1, every=max(1, N // 10))
    ...
    # persist per-code CSVs (incremental)
    ensure_dir(RAW_DIR)
    for code, g in df.groupby("code"):
        p = os.path.join(RAW_DIR, f"{code}.csv")
        write_csv_atomic(p, g, index=False)

    # panel for main window only
    panel = df[(df["date"] >= start) & (df["date"] <= end)].copy()
    cov = CoverageRow(
        date=end.strftime("%Y-%m-%d"),
        codes_total=len(codes),
        codes_in_panel=panel["code"].nunique(),
        rows=len(panel),
    )

    # append coverage with roll-off
    ensure_dir(OUT_TS)
    append_with_rolloff(os.path.join(OUT_TS, os.path.basename(COVERAGE_CSV)),
                        pd.DataFrame([cov.__dict__]),
                        key="date", keep_last=int(ROLLING_KEEP_DAYS))

    log.done(f"[PRE] coverage rows={cov.rows}, codes_in_panel={cov.codes_in_panel}/{cov.codes_total}")
    return cov
...
if __name__ == "__main__":  # pragma: no cover
    ...
    if len(days) >= 20:
        ...
        s, e = (days[0], days[-1]) if days else (today - pd.Timedelta(days=20), today)
    cov = run_preprocess(s, e)
    log.done(f"[SMOKE] preprocess coverage: {cov}")
